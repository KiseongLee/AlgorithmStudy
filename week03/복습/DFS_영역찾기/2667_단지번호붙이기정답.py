...
# 핵심key : dfs를 상하좌우로 진행하면서 0으로 바꿔주고, 하나씩 카운트한다.
#          이 것을 result라는 리스트에 담아가면 문제의 정답이 되겠다.
#          단지 수는 for문 끝나고 세어주면 되겠지.

# 문제의 핵심

# 1. 단지 수는 for문 끝나구 세어주고
# 2. 단지내 집의 수를 오름차순으로 정렬하여 한 줄에 하나씩 --> dfs에서 count로 세어주면 되겠다.