...
def go(now,trace):
    ...
    for i in range(1, n):
        if not trace & (1 << i) and path[now][i] > 0: # 현재 비트마스킹 되어 있는 것과 현재(i)를 비트마스킹 켠 것을 and 조건을 걸었을 때, 만약에 이미 켜져 있는게 있으면 1이 될 것이고 not을 걸면은 0이 되므로 False가 되어서 조건이 걸리지 않고, path[now][i]는 현재에서 i로 가는 비용이 0보다 커야할 때
